...
const FlashSales: React.FC = () => {
  ...
  return (
    <section className="max-w-[1180px] mx-auto my-10 px-2 sm:px-4 md:px-6 select-none">
      <header className="flex flex-col gap-2 sm:gap-2 md:gap-4 mb-8">
        <div>
          <div className="bg-red-600 text-white font-semibold px-4 py-1 rounded-md text-sm whitespace-nowrap inline-block select-none">
            Today&apos;s
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 ml-auto">
          <button
            onClick={prevSlide}
            aria-label="Previous"
            className="w-9 h-9 bg-gray-200 rounded-full grid place-items-center hover:bg-red-600 hover:text-white transition"
          >
            <FaArrowLeft />
          </button>
          <button
      ...
      <div className="flex gap-5 overflow-hidden flex-wrap justify-center md:justify-start">
        {visibleProducts.map((product, idx) => (
          <div
            key={product.id + "-" + idx}
            className="bg-gray-100 rounded-lg p-4 w-[330px] flex-shrink-0 flex flex-col relative shadow-md hover:shadow-lg transition"
          >
            <div className="absolute top-3 left-3 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded select-none">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mb-3">
                <span className="text-red-600 font-bold text-lg select-text">
                  ${product.discountPrice}
                </span>
                <span className="line-through text-gray-500 select-text">
      ...
      <Button className="mt-10 px-6 py-2 max-w-[180px] mx-auto bg-red-600 hover:bg-red-700 text-white rounded font-semibold text-base transition">
        View All Products
      </Button>
    </section>
  );
